    ...
    for (unsigned priorityNum = 0; priorityNum < configMAX_PRIORITIES; priorityNum++) {
        ...
        for (unsigned i = 0; i < uxArraySize; i++) {
            ...
            if (e->uxBasePriority == priorityNum) {
                ...
                output.printf("%10s %s %2u %5u %4u %10u us\n", e->pcTaskName, taskStatusTbl[e->eCurrentState], e->uxBasePriority, stackInBytes, cpuPercent,
                        timeUs);
            }
        }
    }
...
CMD_HANDLER_FUNC(logHandler)
{
    ...
    if (cmdParams == "flush") {
        ...
        output.printf("Call counts    : %u dgb %u info %u warn %u err\n", logger_get_logged_call_count(log_debug), logger_get_logged_call_count(log_info),
                logger_get_logged_call_count(log_warn), logger_get_logged_call_count(log_error));
    }
        ...
        logger_msg_t type = cmdParams.containsIgnoreCase("warn") ? log_warn : cmdParams.containsIgnoreCase("error") ? log_error :
...
CMD_HANDLER_FUNC(cpHandler)
{
    ...
    FRESULT copyStatus = Storage::copy(srcFile, dstFile, &readTimeMs, &writeTimeMs, &bytesTransferred);
...
CMD_HANDLER_FUNC(catHandler)
{
    ...
    if (FR_OK != f_open(&file, cmdParams(), FA_OPEN_EXISTING | FA_READ)) {
        ...
        if (!printToScreen) {
            ...
            output.printf("\nRead %d bytes @ %d Kb/sec", totalBytesRead, totalBytesRead / (0 == timeTaken ? 1 : timeTaken));
        }
        output.putline("");
    }
    return true;
...
CMD_HANDLER_FUNC(lsHandler)
{
    ...
    for (;;) {
        ...
        output.printf("%c%c%c%c%c %u/%02u/%02u %02u:%02u %10lu %13s", (Finfo.fattrib & AM_DIR) ? 'D' : '-', (Finfo.fattrib & AM_RDO) ? 'R' : '-',
                (Finfo.fattrib & AM_HID) ? 'H' : '-', (Finfo.fattrib & AM_SYS) ? 'S' : '-', (Finfo.fattrib & AM_ARC) ? 'A' : '-', (Finfo.fdate >> 9) + 1980,
                (Finfo.fdate >> 5) & 15, Finfo.fdate & 31, (Finfo.ftime >> 11), (Finfo.ftime >> 5) & 63, Finfo.fsize, &(Finfo.fname[0]));
...
CMD_HANDLER_FUNC(mvHandler)
{
    ...
    if (2 != cmdParams.tokenize(" ", 2, &srcFile, &dstFile)) {
        ...
        output.printf("Move '%s' -> '%s' : %s\n", srcFile, dstFile, (FR_OK == f_rename(srcFile, dstFile)) ? "OK" : "ERROR");
    }
    return true;
}
...
CMD_HANDLER_FUNC(dcpHandler)
{
    ...
    for (;;) {
        ...
        if (!(Finfo.fattrib & AM_DIR)) {
            ...
            output.printf("Copy %s -> %s : %d Bytes : %s\n", src(), dst(), Finfo.fsize, (FR_OK == Storage::copy(src(), dst())) ? "OK" : "ERROR");
        }
    }
    return true;
}
...
//        output.putline(FR_OK == Storage::getFlashDrive().mount() ? "Flash mounted" : "Error mounting Flash Memory");
    ...
    return true;
#endif
    //output.putline("Not Yet Implemented");
